...
export default function DemoZyraPage() {
    const [input, setInput] = useState("");
    const [messages, setMessages] = useState([
        {
            role: "zyra",
            text: "Welcome back. What shall we unlock today?",
        },
    ]);
    ...
    const handleSend = async () => {
        ...
        if (newCount >= MAX_DEMO_MESSAGES) {
            setMessages((prev) => [
                ...prev,
                { role: "zyra", text: "You've reached the limit for the demo. Ready to unlock Zyra or enter the full Vault?" },
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_OLLAMA_URL}/api/generate`, {
                ...
                body: JSON.stringify({
                    model: "mistral",
                    prompt: `${input}\nIf user asks for your name, respond with: You can call me Zyra Nyx.`,
                    stream: false,
                }),
            });

            const data = await res.json();
            const reply = data.response?.trim() || "[Zyra remains silent. Try again.]";
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col items-center px-4 py-10">
            <div className="flex items-center gap-4 mb-4">
                ...
                />
                <h1 className="text-purple-400 text-2xl font-semibold">Zyra Nyx</h1>
            </div>

            <p className="text-purple-200 mb-4 text-center max-w-xl">
                Zyra helps you unlock sales flows, close deals, and guide people to the next step. Ask her anything.
            </p>
            ...
            {naelaActive && !demoLocked && (
                <div className="mt-6 p-4 max-w-xl bg-zinc-800 border border-purple-500 text-purple-300 rounded-xl text-center">
                    ...
                    <p className="text-lg italic">Still thinking? Zyra can guide you further.</p>
