...
export default function ZyraPage() {
    ...
    const handleSend = async () => {
        ...
        if (newCount >= MAX_DEMO_MESSAGES) {
            setMessages((prev) => [
                ...prev,
                { role: "zyra", text: "You've just begun to flow. Unlock Tier 2 for full access to all 15 AI's — only €97 lifetime." },
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_OLLAMA_URL}/api/generate`, {
                ...
                body: JSON.stringify({
                    model: "mistral",
                    prompt: `${input}\n\nIf asked for your name, respond as Zyra Nyx, the intuitive sales closer of NexNyx.`,
                    stream: false,
                }),
            });

            const data = await res.json();
            const reply = data.response?.trim() || "[Zyra is thinking. Try again.]";
        ...
        <div className="min-h-screen bg-black text-white px-4 py-10">
            <div className="flex flex-col md:flex-row gap-6 max-w-7xl mx-auto">
                ...
                <div className="md:w-1/4 flex justify-center">
                    <div className="w-40 h-40 rounded-full overflow-hidden border-2 border-purple-500 shadow-lg">
                        <Image
                ...
                <div className="md:w-3/4">
                    <p className="text-purple-400 text-sm mb-2 text-center md:text-left">
                        Zyra is your intuitive sales closer. Ask her anything about DM’s, sales flow, objections or conversions.
                    </p>
                    ...
                    <div className="bg-zinc-900 p-4 rounded-2xl shadow-lg h-[75vh] overflow-y-auto flex flex-col space-y-4">
                        {messages.map((msg, i) => (
                            <div
                                ...
                                className={`whitespace-pre-wrap text-sm md:text-base px-3 py-2 rounded-xl max-w-[80%] ${msg.role === "zyra"
                                    ...
                                    : msg.role === "system"
                                        ? "bg-zinc-600 text-purple-200 text-center self-center"
                        ...
                        {loading && (
                            <div className="text-purple-300 animate-pulse">Zyra is thinking...</div>
                        )}
                        <div ref={chatEndRef} />
                    </div>
                    ...
                    {!demoLocked && (
                        ...
                        >
                            <textarea
                                ...
                                placeholder="Ask Zyra anything about flow, sales, objections..."
                    ...
                    {demoLocked && (
                        <div className="mt-6 flex flex-col items-center gap-4">
                            <button
                                onClick={() => router.push("/vault/pay?ai=zyra")}
                                className="bg-purple-700 hover:bg-purple-800 px-6 py-2 rounded-xl text-white"
                            >
                                🔓 Unlock Zyra – €47/month
                            </button>
                            <button
                    ...
                    {naelaActive && !demoLocked && (
                        <div className="mt-6 p-4 max-w-xl bg-zinc-800 border border-purple-500 text-purple-300 rounded-xl text-center">
                            <p className="text-sm mb-2">Naela Nyx appears...</p>
                            <p className="text-lg italic">Still stuck? Zyra can help you find your next move.</p>
